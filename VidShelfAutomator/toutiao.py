...
async def save_video_info(video_file, title, tags):
    ...
    try:
        ...
        logger.info(f"保存视频信息文件失败: {str(e)}")
    ...
    for video in videos:
        try:
            ...
            if video_file:
                ...
                if not dy_res:
                    logger.info(f"无法解析视频 {aweme_id}，跳过")
                    continue

                # 检查是否为横屏视频
                if not is_horizontal_video(dy_res):
                    logger.info(f"视频 {aweme_id} 不是横屏视频，跳过")
                    continue
                ...
                if not url_list:
                    logger.info(f"无法获取视频 {aweme_id} 的URL，跳过")
                    continue
                ...
                try:
                    ...
                    if not os.path.exists(video_file):
                        logger.info(f"视频下载失败，文件不存在: {video_file}")
                        continue
                except Exception as e:
                    logger.info(f"下载视频时出错: {str(e)}")
                    continue
            ...
            # 在上传前检查文件是否存在
            if not os.path.exists(video_file):
                logger.info(f"视频文件不存在: {video_file}")
                continue
            ...
            if up_state:
                ...
                try:
                    ...
                    logger.info(f"清理文件失败: {str(e)}")
                logger.info(f"账号 {cookie_file} 成功上传视频 {aweme_id}")
                return True  # 成功处理一个视频

        except Exception as e:
            logger.info(f"处理视频时出错: {str(e)}")
            import traceback
...
async def main_process():
    while True:  # 持续运行，直到没有可用账号或用户中断
        ...
        for cookie_file in available_accounts:
            logger.info(f"使用账号 {cookie_file} 处理视频")
