...
def _fake_torch(
    ...
    *,
    ...
    cuda = SimpleNamespace(
        ...
        memory_allocated=_counter(800 * 1024**2, 0),
        memory_reserved=_counter(1000 * 1024**2, 100 * 1024**2),
...
def test_stage_release_records_cuda_and_mps_memory_before_and_after(monkeypatch):
    ...
    assert calls == ["gc.collect", "cuda.empty_cache", "mps.empty_cache"]
    ...
    assert "CUDA allocated 800.0 MiB -> 0.0 MiB" in report.summary()
...
def test_release_failures_are_aggregated_and_raised(monkeypatch):
    ...
    assert calls == ["whisper.release_model", "gc.collect"]
